...
def main(argv):
		...
		if len(boxlog) > 1:
			...
			if load.has_key('DataLeak'):
			    ...
			    load['DataLeak']['tag'] = getTags(int(load['DataLeak']['tag'], 16))
    			    ...
    			    tagCount = len(load['DataLeak']['tag'])
    			    
			    dataleaks[my_time] = load['DataLeak']
			    dataLeaksCount = len(dataleaks)
	...
	shortOutput["servicestart"] = servicestart

	output["recvnet"] = recvnet
	shortOutput["recvnet"] = recvnet
	
	output["opennet"] = opennet
	
	output["sendnet"] = sendnet
	shortOutput["sendnet"] = sendnet
	
	output["closenet"] = closenet

	output["accessedfiles"] = accessedfiles
	shortOutput["accessedfiles"] = accessedfiles
	
	output["dataleaks"] = dataleaks
	shortOutput["dataLeaksCount"] = dataLeaksCount

	output["fdaccess"] = fdaccess
	shortOutput["fdaccess"] = fdaccess
	...
	shortOutput["recvsaction"] = recvsaction
