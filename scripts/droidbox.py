...
def sendEvent():
	...
	subprocess.call("./scripts/sendEvent.sh", shell=True)
'''
def killMe():
	print "In KillME function"
	max_time = int(40)
	print max_time
	start_time = time.time()  # remember when we started
	print start_time
	
	executionTime = start_time + max_time
	while (1):
		print time.time()
		print executionTime 
...
#	timeoutThrd = threading.Thread(target=killMe)
#	timeoutThrd.start()
	
#	timeout timeoutThrd.get()
#	if timeout == 1:
#		sys.exit(1)
	...
	if (ret == 1):
		...
		sys.exit(1)
	if ("Rss" == str(mainActivity)):
		print("Rss Activity Detected, Aborting...")
		sys.exit(1)
	else:
	...
	thrd = threading.Thread(target=sendEvent)
	thrd.start()
	...
	while 1:
		...
		if len(boxlog) > 1:
			...
			if load.has_key('FdAccess'):
			    ...
			    count_accessed_file = len(accessedfiles[load['FdAccess']['id']])
			...
			if load.has_key('DataLeak'):
			    ...
			    load['DataLeak']['tag'] = getTags(int(load['DataLeak']['tag'], 16))
    			    ...
    			    tag_count = len(load['DataLeak']['tag'])
    			    tag_name = load['DataLeak']['tag'] 
    			    
			    dataleaks[my_time] = load['DataLeak']
			    data_leaks_count = len(dataleaks)
	...
	servicestart_found = []
	if servicestart:
		for key,value in servicestart.items():
			servicestart_found.append(value["name"])
			
	shortOutput["servicestart_found"] = servicestart_found

	output["recvnet"] = recvnet
	
	recvnet_found = []
	if recvnet:
		for key,value in recvnet.items():
			recvnet_found.append(value["host"])
			
	shortOutput["recvnet"] = recvnet_found
	...
	sendnet_found = []
	if sendnet:
		for key,value in sendnet.items():
			sendnet_found.append(value["desthost"])

	shortOutput["sendnet"] = sendnet_found

	output["closenet"] = closenet

	output["accessedfiles"] = accessedfiles
	shortOutput["count_accessed_file"] = count_accessed_file
	
	output["dataleaks"] = dataleaks
	shortOutput["data_leaks_count"] = data_leaks_count

	output["fdaccess"] = fdaccess
	
	fdaccess_found = []
	if fdaccess:
		for key,value in fdaccess.items():
			fdaccess_found.append(value["path"])
			
	shortOutput["fdaccess"] = fdaccess_found

	output["sendsms"] = sendsms
	shortOutput["sendsms"] = sendsms
	
	output["phonecalls"] = phonecalls
	shortOutput["phonecalls"] = phonecalls
	...
	recvsaction_found = []
	if fdaccess:
		for key,value in recvsaction.items():
			recvsaction_found.append(value)
#			recvsaction_found.append(value["action"])

	
	shortOutput["recvsaction"] = recvsaction_found
	
	output["enfperm"] = enfperm
	shortOutput["enfperm"] = enfperm

	output["hashes"] = hashes
	
	output["apkName"] = apkName
	shortOutput["apkName"] = apkName
	
	shortOutput["tag_count"] = tag_count
	shortOutput["tag_name"] = tag_name

#	testJson = open("test.json", "w+")

#	testJson.write(json.dumps(output));
#	testJson.close()

#	print(json.dumps(output))
####################### Detailed Output ########################################

	prettyParsed_v = json.dumps(output, indent=4, sort_keys=True)
	print prettyParsed_v

	PackageName = os.path.basename(str(output["apkName"]))
	sha256hash = application.getHashes()[2]

#	logName = str(PackageName) +"-"+ str(sha1hash)+".log"
	logName = str(sha256hash)+".log"

	logVerbose = open("log-comprehensive/"+str(logName), "w+")

#	logVerbose.write(json.dumps(output));
	logVerbose.write(prettyParsed_v);
	logVerbose.close()
	
####################### Summery Output ########################################

	prettyParsed_s = json.dumps(shortOutput, indent=4, sort_keys=True)
#	print prettyParsed_s
...
#	logName = str(PackageName) +"-"+ str(sha1hash)+".log"
	...
	logVerbose_s = open("log-summaries/"+str(logName), "w+")

#	logVerbose.write(json.dumps(output));
	logVerbose_s.write(prettyParsed_s);
	logVerbose_s.close()
